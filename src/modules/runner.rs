...
/// Command execution engine that runs both custom and system commands.
///
/// The `Runner` is responsible for executing commands by first checking if a custom
/// implementation exists in the configured binary path, and falling back to system
/// executables if not found. It handles environment variables, I/O redirection
/// (stdin, stdout, stderr), and both append and write modes for output files.
///
/// # Architecture
///
/// - Custom binaries are checked first in the `bin_path` directory
/// - If not found, falls back to system PATH executables
/// - Supports full I/O redirection with file paths
/// - Passes environment variables to all executed processes
///
/// # Examples
///
/// ```
/// use std::path::PathBuf;
/// use cli_rust::modules::{command::Command, runner::Runner, Environment};
///
...
impl Runner {
    /// Creates a new Runner with the specified binary path.
    ///
    /// # Arguments
    ///
    ...
    /// Executes a command, using custom binary if available or system executable otherwise.
    ///
    /// This method first checks if a custom implementation exists in the `bin_path` directory.
    /// If found, it executes the custom binary. Otherwise, it attempts to execute the command
    ...
    /// # Returns
    ///
    /// - `Ok(String)` - The command's output (stdout) if successful
    /// - `Err(io::Error)` - If execution fails
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// Executes a custom binary from the configured bin directory.
    ///
    /// This internal method handles executing custom command implementations,
    /// setting up environment variables, handling I/O redirections, and
    /// processing stdin/stdout/stderr according to the command configuration.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// - `Ok(String)` - Command output or empty string if redirected to file
    /// - `Err(io::Error)` - If execution or I/O operations fail
